...
# Test
def testDefed(denoiser, classifier, testLoader, vision_criterion=nn.MSELoss(), class_criterion=nn.CrossEntropyLoss(), noise_level=0., alpha=1.):
    total = 0.
...
# Train Denoiser
def trainDefed(denoiser, classifier, trainLoader, testLoader, optimizer, scheduler, epochs, dataset, edge_method: str = 'laplacian',
             vision_criterion=nn.MSELoss(), class_criterion=nn.CrossEntropyLoss(), noise_level=0., alpha=1., save: bool = True):
    ...
    class_best_path_gpu = Dir + dataset + r'-{}-alpha={}-noise={}-bestclass-GPU.pth'.format(edge_method, alpha, noise_level)
    vision_best_path_gpu = Dir + dataset + r'-{}-alpha={}-noise={}-bestvision-GPU.pth'.format(edge_method, alpha, noise_level)
    loss_best_path_gpu = Dir + dataset + r'-{}-alpha={}-noise={}-bestloss-GPU.pth'.format(edge_method, alpha, noise_level)
    
    class_best_path_cpu = Dir + dataset + r'-{}-alpha={}-noise={}-bestclass-CPU.pth'.format(edge_method, alpha, noise_level)
    vision_best_path_cpu = Dir + dataset + r'-{}-alpha={}-noise={}-bestvision-CPU.pth'.format(edge_method, alpha, noise_level)
    loss_best_path_cpu = Dir + dataset + r'-{}-alpha={}-noise={}-bestloss-CPU.pth'.format(edge_method, alpha, noise_level)
    
    train_log_path = Dir + r'log/trainLog-{}-{}-alpha={}-noise={}.pt'.format(dataset, edge_method, alpha, noise_level)
    test_log_path = Dir + r'log/testLog-{}-{}-alpha={}-noise={}.pt'.format(dataset, edge_method, alpha, noise_level)
    ...
    for epoch in range(epochs):
        ...
        test_vision_loss, test_class_loss, test_loss, test_acc, test_mse, test_psnr, test_ssim = testDefed(denoiser=denoiser,
...
# Main
def main():
    # python3 Defed-Train.py -d mnist -s laplacian -a 1 -n 0 10 20 -e 10 -b 256 -g 1,2 -l -p
    ...
    sys.stdout = utils.Logger(r'./Defed/trainLog/trainLog-{}-{}-{}.txt'.format(dataset, alphas[0], structure))
    ...
    for alpha in alphas:
        for noise_level in noiseLevels:
            denoiser = Defed(num_channels=num_channels, edge_method=structure).cuda()
            ...
            test_before_train = testDefed(denoiser=denoiser,
                                        classifier=classifier, 
                                        testLoader=testLoader, 
                                        vision_criterion=vision_criterion, 
                                        class_criterion=class_criterion, 
            ...
            print('*****'*6 + '\t' + 'Denoiser {} Alpha: {} Noise Level: {} Train Begin'.format(structure, alpha, noise_level) + '\t' + '*****'*6)
            
            trainDefed(denoiser=denoiser,
                     classifier=classifier, 
            ...
            print('*****'*6 + '\t' + 'Denoiser {} Alpha: {} Noise Level: {} Train Finish'.format(structure, alpha, noise_level) + '\t' + '*****'*6)
